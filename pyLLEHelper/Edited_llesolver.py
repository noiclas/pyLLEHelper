...
path_juliaScript = os.path.join(path_juliaScript, "EditedComputeLLE.jl") 
'''

HELLO, THE ONLY CHANGE TO THIS WHOLE FILE IS THIS LINE HERE ^^^^^^^^

its quite silly to have a whole new file just for this one change.
to reduce the number of files, maybe just have some checking system
in the OG file to have the path go to the desired .jl file
...
class Latexify:
    ...
    __credits__ = ["Gregory Moille", "Kartik Srinivasan"]
    __license__ = "GPL"
    __version__ = "2.0.1"
...
class LLEsolver(object):
    ...
    **debug <bool>**: Save a trace in a logfile in the working directory of the different actions pyLLE perform (default = False)
    ...
    __author__ = "Gregory Moille"
    __copyright__ = "Copyright 2018, NIST"
    __credits__ = ["Gregory Moille", "Qing Li", "Xiyuan Lu", "Kartik Srinivasan"]
    __license__ = "GPL"
    __version__ = "1.0.0"
    __maintainer__ = "Gregory Moille"
    __email__ = "gregory.moille@nist.gov"
    ...
    def __init__(self, **kwargs):
        ...
        assert len(self._sim["Pin"]) == len(
            self._sim["f_pmp"]
        ), "The number of *pump* and *pump power* has to be the same"
        ...
        if not np.diff(self._sim["mu_sim"])[0] % 2 == 0:
            ...
            print(
                "μ_sim = [{:.0f} {:.0f}]".format(
                    self._sim["mu_sim"][0], self._sim["mu_sim"[1]]
                )
            )
        ...
        assert len(self._sim["f_pmp"]) == len(
            self._sim["domega"]
        ), "Please provide the same number of pump fixed detuning than pump frequencies. Detuning = None for the pump which is swept"
        assert len(self._sim["Pin"]) == len(
            self._sim["domega"]
        ), "Please provide the same number of pump fixed detuning than pump power. Detuning = None for the pump which is swept"
    ...
    def Analyze(
        self, plot=True, display_center=True, label=None, plottype="all", verbose=False
    ):
        """
        ...
        self._analyze = AnalyzeDisp(
            file=self._res["dispfile"],
            fpmp=fpmp,
        ...
        self._sim["mu_sim_center"] = [
            self._sim["mu_sim"][0] + self._sim["ind_pmp"][0],
    ...
    def Setup(self, tmp_dir=None, verbose=False):
        ...
        def LoggerDic():
            dic_sim = {
                ...
                "domega_aux": ("\u03B4\u03C9_aux", 1e-9 / (2 * np.pi), "x2\u03C0 GHz"),
            ...
            for k, it in self._res.items():
                if k in dic_res.keys():
                    if it == None:
                        ...
                        Info += "\t\t{} = {:.2f} {}\n".format(
                            dic_res[k][0], it * dic_res[k][1], dic_res[k][2]
                        )
            ...
            for k, it in self._sim.items():
                if k in dic_sim.keys():
                    if type(it) is list:
                        if (
                            dic_sim[k][0] == "\u03BC_sim"
                            or dic_sim[k][0] == "\u03BC_fit"
                        ):
                            if it == [None, None]:
                                Info += "\t\t{} = [None,None] {}\n".format(
                                    dic_sim[k][0], dic_sim[k][2]
                                )
                            else:
                                Info += "\t\t{} = [{:.2f},{:.2f}] {}\n".format(
                                    dic_sim[k][0],
                        ...
                        if it == None:
                            Info += "\t\t{} = {} {}\n".format(
                                dic_sim[k][0], "None", dic_sim[k][2]
                            )
                        else:
                            Info += "\t\t{} = {:.2f} {}\n".format(
                                dic_sim[k][0], it * dic_sim[k][1], dic_sim[k][2]
                            )
        ...
        def SetupHDF5():
            ...
            if verbose:
                print(
                    "HDF5 parameter file can be foud in: {}".format(
                        self.tmp_dir + "ParamLLEJulia.h5"
                    )
                )
    ...
    def SolveTemporal(
        self, verbose=False, tol=1e-3, maxiter=6, step_factor=0.1, bin=None
    ):
        """
        ...
        def DisplaySim():
            if self._debug:
                self._logger.info(
                    "LLEsovler.SolveTemporal", "Solving Temporal LLE with Julia...."
                )
                Info = "tol = {} -- maxiter = {} step_factpr = {}".format(
                    tol, maxiter, step_factor
                )
            ...
            ) + "{:0>2}:{:0>2}:{:0>2}".format(date.tm_hour, date.tm_min, date.tm_sec)
        ...
        def LaunchJulia(bin):
            ...
            try:
                ...
                print(err)
                raise ValueError(
                    "julia is not installed on the system path. ",
        ...
        def ProbeProgress(fname, start):
            ...
            if not err:
                    ...
                    try:
                        ...
                        try:
                            ...
                            if not perc_old == perc:
                                line, length, pgrs, tb_up = Pbar(
                                    perc, pgrs, tb_up, conv_err
                                )
                                print("\r" + line, end="")
                                perc_old = perc
                ...
                time_taken = "Simulation Time " + "{:0>2}h:{:0>2}min:{:0>4.1f}s".format(
    ...
    def SolveSteadyState(self, do_plot=True):
        ...
        ut0 = np.sqrt(α / γ / L) * (
            Φ0
            + B0 * np.exp(1j * φ0) * sech(0.5 * B0 * np.sqrt(α / (np.abs(β2) * L)) * τ)
        )
        ...
        fm = (
            lambda xx: φ * Em(xx)
            + nlc * fft.fft(np.abs(Ut(xx)) ** 2 * Ut(xx))
            + Ein_couple
        )
        ...
        if not pyType == "jupyter":
            if do_plot:
                ...
                ax.plot(
                    1e-12 * ν, 30 + 10 * np.log10(np.abs(Ewg) ** 2), label="Waveguide"
                )
            ...
            if do_plot:
                trace0 = go.Scatter(
                    x=1e-12 * ν,
                    y=30 + 10 * np.log10(np.abs(Ering) ** 2),
                    mode="lines",
                    name="Res. Power",
                )
                trace1 = go.Scatter(
                    x=1e-12 * ν,
                    y=30 + 10 * np.log10(np.abs(Ewg) ** 2),
                    mode="lines",
                    name="Out Power",
                )
                data = [trace1, trace0]
    ...
    def RetrieveData(self):
        ...
        with h5py.File(self.tmp_dir + "ResultsJulia.h5", "r") as S:
            if self._debug:
                self._logger.info(
                    "LLEsovler.RetrieveData",
                    "Retrieving results from Julia in {}".format(
                        self.tmp_dir + "ResultsJulia.h5"
                    ),
                )
            self._sol = {}
        ...
        μcenter = np.linspace(
            self._sim["mu_sim_center"][0], self._sim["mu_sim_center"][1], μlength
        )
        ...
        for ii in range(u_probe.shape[1]):
            ...
            Ewg[:, ii] = (
                np.fft.fftshift(np.fft.fft(Awg[:, ii])) / np.sqrt(μlength) + 1e-12
            )
            Ecav[:, ii] = np.fft.fftshift(np.fft.fft(Acav[:, ii])) / np.sqrt(μlength)
            Pcomb[ii] = np.sum(np.abs(Ecav[:, ii]) ** 2, 0) - np.sum(
                [
                    np.abs(Ecav[μcent + jj, ii]) ** 2
                    for jj in np.unique(self._sim["ind_pmp"])
                ]
            )
        ...
        sol = {
            "δfreq": self._sol["detuning"] / (2 * np.pi),
            "time": self._sol["time"],
            "Awg": Awg,
            "Acav": Acav,
            "Ewg": Ewg,
    ...
    def PlotSpectraMap(self, do_matplotlib=False):
        tr = [
            go.Heatmap(
                z=10 * np.log10(np.abs(self.sol.Ewg) ** 2) + 30,
                zmax=0,
                zmin=-90,
    ...
    def PlotCombSpectra(
        ...
        if xaxis.lower() == "frequencies" or xaxis.lower() == "freq":
            ...
            x = (
                np.arange(self._sim["mu_sim"][0], self._sim["mu_sim"][-1])
                - self._sim["ind_pmp"][0]
            )
            xlabel = "Mode Numbers"
        ...
        if not pyType == "jupyter" or do_matplotlib:
            ...
            tr = [go.Scatter(x=x, y=y, name=name[0])]
            if where == "both":
                tr += [go.Scatter(x=x, y=y2, name=name[1])]
    ...
    def PlotSolitonTime(self, ind, f=None, ax=None, label=None, do_matplotlib=False):
        ...
        **Output**
        ...
        if not pyType == "jupyter" or do_matplotlib:
            ...
            trace0 = go.Scatter(x=τ * 1e12, y=U, mode="lines")
            data = [trace0]
            layout = dict(
    ...
    def __repr__(self):
        ...
        if "Tscan" in self._sim:
            sim_table.add_row(
                ["Tscan", "{:.3f}".format(self._sim["Tscan"] * 1e-5), "x1e5 tR"]
            )
        if "domega_init" in self._sim:
            sim_table.add_row(
                ["δω_init", "{:.3f}".format(self._sim["domega_init"] * 1e-9), "GHz"]
            )
        if "domega_end" in self._sim:
            sim_table.add_row(
                ["δω_end", "{:.3f}".format(self._sim["domega_end"] * 1e-9), "GHz"]
            )
...
class _dic2struct:
    ...
    def reprRes(self):
        ...
        try:
            table.add_row(
                ["γ", self._dic["gamma"], "Non-linear coefficient", "W^-1 m^-1"]
            )
        except:
    ...
    def reprSim(self):
        ...
        table.add_row(
            ["f_center", f_center + " (THz)", "Center of the sim domain", "Hz"]
        )
        ...
        δω = "[" + ", ".join(δω) + "]"
        table.add_row(
            ["δω_init", δω_init + " (GHz)", "Start of the frequency sweep", "x2π Hz"]
        )
        table.add_row(
            ["δω_end", δω_end + " (GHz)", "End of the frequency sweep", "x2π Hz"]
        )
        ...
        try:
            ...
            table.add_row(
                [
                    "μcenter",
                    self._dic["μcenter"],
                    "Index of the center of the sim domain",
                    "",
                ]
            )
        except:
        ...
        table.add_row(
            [
                "ind_pmp",
                self._dic["ind_pmp"],
                "Pump index relative to center of domain",
                "",
            ]
        )
        table.add_row(
            [
                "ind_pump_sweep",
                self._dic["ind_pump_sweep"][0],
                "Pump index to sweep",
                "",
            ]
        )
        table.add_row(["num_probe", self._dic["num_probe"], "Sampling number", ""])
        table.vrules = False
        table.header = True
        table.align = "l"
        table.padding_width = 2
    ...
    def reprSol(self):
        ...
        try:
            δfreq = "[{:.3f} ... {:.3f}]".format(
                self._dic["δfreq"][0] * 1e-9, self._dic["δfreq"][-1] * 1e-9
            )
        except:
            δfreq = "[{:.3f} ... {:.3f}]".format(
                self._dic["dfreq"][0] * 1e-9, self._dic["dfreq"][-1] * 1e-9
            )

        table.add_row(["δfreq", δfreq + " (GHz)", "Pump detuning", "Hz"])

        time = "[{:.3f} ... {:.3f}]".format(
            self._dic["time"][0] * 1e6, self._dic["time"][-1] * 1e6
        )
        ...
        Pcomb = "[{:.3f} ... {:.3f}]".format(
            self._dic["Pcomb"][0] * 1e3, self._dic["Pcomb"][-1] * 1e3
        )
        table.add_row(["Pcomb", Pcomb + " (mW)", "Intra-cavity comb power", "W"])

        Pwg = "[{:.3f} ... {:.3f}]".format(
            self._dic["Pwg"][0] * 1e3, self._dic["Pwg"][-1] * 1e3
        )
        table.add_row(["Pwg", Pwg + " (mW)", "In-waveguide power", "W"])

        Pcav = "[{:.3f} ... {:.3f}]".format(
            self._dic["Pcav"][0] * 1e3, self._dic["Pcav"][-1] * 1e3
        )
        ...
        freq = "[{:.3f} ... {:.3f}]".format(
            self._dic["freq"][0] * 1e-12, self._dic["freq"][-1] * 1e-12
        )
    ...
    def reprDisp(self):
        ...
        freq = "[{:.3f} ... {:.3f}]".format(
            self._dic["freq"][1] * 1e-12, self._dic["freq"][-2] * 1e-12
        )
        table.add_row(["freq", freq + " (THz)", "Mode frequency", "Hz"])
        freq_sim = "[{:.3f} ... {:.3f}]".format(
            self._dic["freq_sim"][1] * 1e-12, self._dic["freq_sim"][-2] * 1e-12
        )
        table.add_row(
            [
                "freq_sim",
        ...
        Dint0 = "[{:.3f} ... {:.3f}]".format(
            self._dic["Dint0"][1] * 1e-9, self._dic["Dint0"][-2] * 1e-9
        )
        table.add_row(
            ["Dint0", Dint0 + " (GHz)", "Dint at the center of sim domain", "Hz"]
        )
