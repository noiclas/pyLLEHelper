...
function Loadh5Param(dir)
    ...
    sim = Dict()
    sim_name = ["res", "sim"]
    par = [["Qc", "R", "ng", "Qi", "gamma","dispfile"], ["Pin", "Tscan", "domega_init", "domega_end", "domega_stop", "domega", "f_pmp", "mu_sim_center", "ind_pmp", "Dint", "ind_pump_sweep","f_center", "phi_pmp", "D1", "DKSinit_imag","DKSinit_real", "num_probe", "Manual_Detuning","Manual_Pin"]]
    cnt = 1
    for sim_par = [res, sim]
        for it = par[cnt]
...
# -- NIC'S ADDITIONS
# ----------------------------------------------------
Manual_Detuning = sim["Manual_Detuning"]
Manual_Detuning_Vect = collect(Manual_Detuning)
Manual_Pin = sim["Manual_Pin"]
if length(Manual_Pin) == 1
    Manual_Pin_Vect = ones(Int(Nt))
elseif length(Manual_Pin) == Nt
    Manual_Pin_Vect = sqrt.(collect(Manual_Pin)/Ppmp[1])
else
    1/0
end
# ----------------------------------------------------
...
if length(Manual_Detuning_Vect) == 1
    for ii in ind_sweep
...
elseif length(Manual_Detuning_Vect) == Int(Nt)
    for ii in ind_sweep
        δω_all[ii][:] = Manual_Detuning_Vect
    end
else 
    1/0
end
...
function Fdrive(it)
    ...
    for ii=1:length(fpmp)
        ...
        Force .= Force .- 1im .* Ain[ii].* Manual_Pin_Vect[it] .*exp(1im .*σ)
    end
